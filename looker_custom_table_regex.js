looker.plugins.visualizations.add({
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    // ULEPSZONA, "KULOODPORNA" FUNKCJA WYSZUKIWANIA NAZWY KOLUMNY
    const getRealFieldName = (inputStr) => {
      if (!inputStr) return null;
      const searchStr = inputStr.toLowerCase().trim();
      
      // 1. Standardowe szukanie po etykietach (działa, jeśli Looker ich nie usunął)
      ...
      // 2. Agresywne dopasowanie bezpośrednio po surowych danych
      if (data.length > 0) {
        const rowKeys = Object.keys(data[0]);
        // Usuwamy z szukanego tekstu wszystko, co nie jest literą (w tym polskie znaki) lub cyfrą
        const simplifiedSearch = searchStr.replace(/[^a-z0-9ąćęłńóśźż]/gi, '');
        
        const keyMatch = rowKeys.find(k => {
          // Looker nadaje klucze w formacie: "nazwa_widoku.nazwa_pola". Wyciągamy samą "nazwa_pola"
          const fieldPart = k.split('.').pop();
          const simplifiedKey = fieldPart.replace(/[^a-z0-9ąćęłńóśźż]/gi, '');
